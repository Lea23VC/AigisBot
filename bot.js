...
client.on("ready", () => {
  ...
  client.user.setActivity("!halp for rom searching commands", {
    type: "PLAYING",
  });
});
  ...
  switch (true) {
    ...
    case msg.content.startsWith("!avatar"):
      const avatarList = msg.mentions.users.map((user) => {
        //convierte el mapa de usuarios en una lista, lo cual recorre con la variable "user"
    ...
    case msg.content === "!ps2 shin megami tensei marselo" ||
      msg.content === "!ps2 Shin Megami Tensei marselo" ||
      msg.content === "!ps2 shin megami tensei Marselo" ||
      msg.content === "!ps2 Shin Megami Tensei Marselo":
      embedIMG("https://i.imgur.com/W0PC2nA.png", msg);
      break;

    case msg.content === "Asi pasa" ||
      msg.content === "asi pasa" ||
      msg.content === "Así pasa" ||
      msg.content === "así pasa":
    ...
    case msg.content.startsWith("!switch"):
      msg.channel.send(
        "Important: Does not contain important first party titles like Super Mario Oddyssey, Super Smash Bros. Ultimate, etc"
      );
      var res = msg.content.slice(8);
      getURL(msg, res, "15AP4RRsZ8TujFbeE4riHpuo6w8K4oT7I");
      break;

    case msg.content.startsWith("!genesis") ||
      msg.content.startsWith("!megadrive"):
      var res;
...
async function saucenao(img, msg) {
  ...
  const cliente = sagiri("3397773ef599feb5954db044e6e89e377434dd4b", {
    results: 10,
  });
  console.log("WRYYYY");
...
function getURL(msg, names, folderId) {
  ...
  if (checkCommand(names, msg)) {
    ...
    drive.files.list(
      {
        q: "name contains '" + names + "' and '" + folderId + "' in parents",
      ...
      (err, res) => {
        ...
        if (files.length) {
          ...
          if (nasos === 1) {
            msg.author.send(
              "Download https://drive.google.com/file/d/1iU8WMD4hziVjtpGvhnfd2JfFlpeFhWv3/view?usp=sharing and extract the files into a folder. To extract a rom drag the rom onto the .exe, a command prompt should popup and extract the ISO from the .dec file."
            );
            nasos = 0;
          }
          //return files;
...
function halp() {
  let embed = new Discord.RichEmbed()
    ...
    .addField(
      "Gamecube (NKit GCZ)",
      "`!gamecube` or `!gc`. `!gc iso` for ISOs (USA only)"
    )
    ...
    .addField(
      "Nintendo 3DS (cia only)",
      "`!3ds`, `!3ds vc` for Virtual Console games"
    )
...
async function memeRepellent(abajo, msg) {
  ...
  imgflipper.generateMeme(383191434, abajo, abajo, (err, img) => {
    if (err) return console.log(err);
